#include "TextBox.h"
// Constractor takes x & y position and length of component.
TEXTBOX::TEXTBOX(int x, int y, int length) :COMPONENT(x, y, length){
	this->valPtr = 0;
	this->position.X = x + 1;
	this->position.Y = y + 1;
	this->cursor.X = x + 1;
...
// Takes the cursor to the place where the input is edit on the component.
...
void TEXTBOX::SetFocus(COORD *coord){
	if (coord != NULL){
		this->focus = true;
		FocusOnText(*coord);
	}
